...
contract ProfileTokenURI {
    ...
    function getProfileTokenURI(
        ...
        string memory usernameWithAtSymbol = string(
            abi.encodePacked("@", username, ".fren")
        );
        return
            string(
                abi.encodePacked(
                    ...
                    Base64.encode(
                        abi.encodePacked(
                            '{"name":"',
                            usernameWithAtSymbol,
                            '","description":"',
                            usernameWithAtSymbol,
                            ' - Frenly profile","image":"data:image/svg+xml;base64,',
                            ...
                            '"},{"trait_type":"frenship_status","value":"',
                            _handleFrenshipStatus(frenshipStatus),
    ...
    function _getSVGImageBase64Encoded(
        string memory usernameWithAtSymbol,
        string memory imageURI
                ...
                abi.encodePacked(
                    ...
                    Strings.toString(
                        _handleLengthToFontSize(
                            bytes(usernameWithAtSymbol).length
                        )
                    ),
    ...
    function _getSVGProfilePicture(
        ...
        if (_shouldUseCustomPicture(imageURI)) {
            return
                string(
                    abi.encodePacked(
                        '<image id="custom-picture" image-rendering="pixelated" preserveAspectRatio="xMidYMid slice" height="450" width="450" xlink:href="',
                        imageURI,
                        '"></image>'
                    )
                );
    ...
    function _handleFrenshipStatus(
        uint8 handleStatus
